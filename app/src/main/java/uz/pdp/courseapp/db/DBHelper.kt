...
class DBHelper(context: Context) :
    ...
    override fun getAllOpeningGroups(courseId: Int): ArrayList<Group> {
        val database = this.readableDatabase "SELECT * FROM " + Constant.GROUP_TABLE + " WHERE " + Constant.GROUP_COURSE_ID + " = ? AND " + Constant.IS_OPENED + " = ?"
        val cursor = database.rawQuery(query, a
        val list = ArrayList<Group>()
        val query =
            "SELECT * FROM " + Constant.GROUP_TABLE + " WHERE " + Constant.GROUP_COURSE_ID + " = ? AND " + Constant.IS_OPENED + " = ?"
        val cursor = database.rawQuery(query, arrayOf("$courseId", "0"))
        if (cursor.moveToFirst()) {
            do {
                val index = cursor.getInt(0)
                val name = cursor.getString(1)
